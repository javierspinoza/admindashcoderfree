                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize text-center ps-3">BARRIOS REGISTRADOS</h6>
                            </div>
                        </div>
                        <div class="row">
                                            ...
                                            @foreach ($barrios as $barrio)
                                                <tr>
                                                    <td class="td-actions text-center">
                                                        <button wire:click="edit({{ $barrio->id }})"
                                                        ...
                                                        <button
                                                            wire:click.prevent="$emit('destroyBarrio', {{ $barrio->id }})"
                                                    ...
                                                    <td style="padding-left: 25px">{{ $barrio->id }}</td>
                                                    <td style="padding-left: 25px">{{ $barrio->name }}</td>
                                                    <td style="padding-left: 25px">{{ $barrio->ciudades->name }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="input-group input-group-static mb-4 mt-2" wire:ignore>
                                            ...
                                            <select class="form-control js-example-basic-single-actualizar" wire:model="ciudad_id">
                                                @foreach ($ciudades as $ciudad)
                                                    <option value="{{ $ciudad->id }}">{{ $ciudad->name }}
                                                    </option>
                                                @endforeach
...
@push('mijs')
    ...
    <script>
        // destroyBarrio es el nombre del metodo del boton ---- barrio_id nombre del parametro que pasamos en la confirmacion de mas abajo
        Livewire.on('destroyBarrio', barrio_id => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // barrios es el nombre de la vista----- destroy el metodo del controlador--- barrio_id el que pasamos al inicio de este script
                    Livewire.emitTo('barrios', 'destroy', barrio_id)
                    Swal.fire({
                        position: 'center',
                        icon: 'success',
